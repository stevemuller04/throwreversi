...
Game.prototype.refresh = function () {
	var self = this;
	ajax("/board", "GET", {}, function (data) {
		self.godmode = data.godmode;
...
Game.prototype.buildBoard = function() {
	var self = this;
	var nBoard = document.getElementById("board");
	for (var y = 0; y < this.board.length; y++)
	{
		var nRow = document.createElement("div");
		nRow.className = "boardrow";
		for (var x = 0; x < this.board[y].length; x++)
		{
			var nCol = document.createElement("div");
			var nBtn = document.createElement("button");
			nBtn.setAttribute("type", "button");
			nBtn.setAttribute("bx", x);
			nBtn.setAttribute("by", y);
			nBtn.onclick = function() { self.move(this.getAttribute("bx"), this.getAttribute("by")); };
			nCol.appendChild(nBtn);
			nRow.appendChild(nCol);
		}
		nBoard.appendChild(nRow);
	}
};
Game.prototype.updateBoard = function() {
	...
	var nBtnP0 = document.getElementById("btnP0");
	var nBtnGM = document.getElementById("btnGM");
	nBtnP1.className = this.player == 1 ? "blue x" : "";
...
var G = new Game();
document.addEventListener("DOMContentLoaded", function() { G.refresh(); });
</script>
<title>ThrowReversi</title>
</head>
<body class="container">
	<div class="row">
		...
		<p>
			...
			<button type="button" id="btnP2" onclick="G.setPlayer(2)">red</button>
